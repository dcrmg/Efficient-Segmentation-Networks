...
class ADE20KSegmentation(SegmentationDataset):
    ...
    def __init__(self, dataset_root='/media/sdb/datasets/segment', split='train', mode=None, transform=None, crop_size=416, encode=False, **kwargs):
        ...
        assert os.path.exists(dataset_root), "dataset root {0} doesnot exits! " \
                                                              "Please setup the dataset using ../datasets/ade20k.py".format(dataset_root)
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            img = self.transform(img)
        return img, mask, img.size(), os.path.basename(self.images[index])
